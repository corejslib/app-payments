...
export default class Blockchain {
    ...
    async getTransactions ( blockNumber ) {
        return this.#api.trx.getBlockRange( blockNumber, blockNumber + 99 );
    }

    decodeTransaction ( transaction ) {
        const ret = transaction.ret?.[ 0 ];
        if ( ret?.contractRet !== "SUCCESS" ) return;

        const id = transaction.txID,
            date = new Date( transaction.raw_data.timestamp ),
        ...
        if ( type === "TransferContract" ) {
            return {
                ...
                date,
                type,
                "fromAddress": tronweb.utils.address.fromHex( parameter.owner_address ),
                "toAddress": tronweb.utils.address.fromHex( parameter.to_address ),
            ...
            const signature = parameter.data.slice( 0, 8 ),
            ...
            const coder = AbiCoder.defaultAbiCoder(),
                ...
                values = coder.decode( decodeTypes, "0x" + parameter.data.slice( 8 ) );
            ...
            return {
                ...
                type,
                "fromAddress": tronweb.utils.address.fromHex( parameter.owner_address ),
                "contractAddress": tronweb.utils.address.fromHex( parameter.contract_address ),
                "method": method.name,
